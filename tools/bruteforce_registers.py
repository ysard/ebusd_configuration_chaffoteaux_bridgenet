...
def read_bruteforce(wanted_combinations=tuple(), boiler_active=False):
    """Search registers for which values are dependent on boiler status

    :param wanted_combinations: Search only in this iterable of registers.
    ...
    :type wanted_combinations: <iterable>, <set> or <list> or <tuple>
    ...
    for index, (xx, yy) in enumerate(it.product(range(byte1_end), range(byte2_end)), 1):
        ...
        if current_register in not_wanted_combinations:
            continue
        if wanted_combinations and current_register not in wanted_combinations:
            continue
...
if __name__ == "__main__":
    ...
    potential_registers = read_bruteforce(
        wanted_combinations=wanted_combinations,
        boiler_active=False
    )
    ret = input(
        Fore.LIGHTGREEN_EX +
